...
-- Policy for players to view their own logs
CREATE POLICY "Players can view their own logs" ON player_logs
  FOR SELECT USING (auth.uid() = player_id);

-- Policy for inserting logs (allow authenticated users)
CREATE POLICY "Players can insert their own logs" ON player_logs
