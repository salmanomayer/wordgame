...
export default function ScoresPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (json?.stats) setStats(json.stats as PlayerStats)
      if (Array.isArray(json?.recent_games)) setSessions(json.recent_games)
      if (Array.isArray(json?.subject_stats)) setSubjectStats(json.subject_stats)
                    ...
                    {sessions.length === 0 ? (
                      ...
                      sessions.map((session) => (
                        <TableRow key={session.id} className="border-slate-700">
                          <TableCell className="font-medium text-white">{session.subject_name}</TableCell>
                          <TableCell>
                            <Badge
                              variant={
