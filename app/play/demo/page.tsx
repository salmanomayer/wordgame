...
function generateOptionsWithRng(correctLetter: string, rng: () => number): string[] {
  ...
  const wrongPool = allLetters.filter((l) => l !== correct)
  const wrongOptions = shuffleWithRng(wrongPool, rng).slice(0, 3)
  return shuffleWithRng([correct, ...wrongOptions], rng)
}
...
export default function DemoPage() {
  ...
  const puzzle = useMemo(() => {
    ...
    const rng = mulberry32(fnv1a32(`${gameNumber}|${currentIndex}|${word}`))
    const missingIdx = Math.floor(rng() * word.length)
    const correctLetter = word[missingIdx]
    const options = generateOptionsWithRng(correctLetter, rng)
    return { missingIdx, options }
  }, [currentIndex, gameNumber])
  ...
  const handleAnswer = (selectedLetter: string) => {
    ...
    const correctLetter = currentWord.word[puzzle.missingIdx]
  ...
  const wordDisplay = currentWord.word.split("").map((letter, idx) => (idx === puzzle.missingIdx ? "_" : letter))
          ...
          <CardContent className="p-4 sm:p-8">
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
              {puzzle.options.map((letter) => {
                const correctLetter = currentWord.word[puzzle.missingIdx].toUpperCase()
